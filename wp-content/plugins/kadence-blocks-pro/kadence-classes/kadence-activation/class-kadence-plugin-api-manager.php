...
update_option( 'kt_api_manager_ascend_premium_activated',true );
...
if ( ! class_exists( 'Kadence_Plugin_API_Manager' ) ) {
	...
	class Kadence_Plugin_API_Manager {
			...
			if ( is_null( self::$memberactive ) ) {
				...
				if ( 'Pinnacle Premium' == $current_theme_name || 'pinnacle_premium' == $current_theme_template ) {
					// Check if activated
						...
						$data[ 'kt_api_key' ] = 'ktm-nullmasterinbabiato';
		...
		public function license_key_deactivation( $p_key ) {
			...
			$args = array('email'       => 'nullmaster@babiato.org' ,'licence_key' => 'ktm-nullmasterinbabiato' ,'product_id'  => '100',);
		...
		public function get_data_options( $key ) {
			self::$settings[ $key ] = $this->get_setting_option( $key, array( 'api_email' => 'nullmaster@babiato.org', 'api_key' => 'ktm-nullmasterinbabiato' ) );
		...
		public function validate_options( $input ) {
			$current_key = 'ktm-nullmasterinbabiato';
